...
Your Go teammate writes the WASM module and tests. You import the generated `client.ts` class. That's it.
...
See the [example/web/app.ts](../example/web/app.ts) for a working TypeScript application that uses the generated class API.
...
This creates `client.ts` (and `worker.js` by default).
...
const wasm = await Wasm.init('./example.wasm');  // init is still async
...
│   ├── index.html
│   ├── app.js            # Bundled TypeScript (includes client.ts)
│   ├── worker.js         # Generated Web Worker
│   └── example.wasm      # Compiled Go code
└── client.ts             # Generated TypeScript class (dev only)
```
...
const wasm = Wasm.init('./worker.js');
