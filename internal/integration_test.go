...
func TestFullPipeline_Primitives(t *testing.T) {
	...
	output := generator.Generate(sigs, packageName)
...
func TestFullPipeline_AllIntegration(t *testing.T) {
	...
	if !strings.Contains(output, "// client.ts - Generated by gowasm-bindgen --sync") {
		t.Errorf("expected header comment, got:\n%s", output)
	}

	// Verify class structure
	if !strings.Contains(output, "export class") {
		t.Errorf("expected export class, got:\n%s", output)
	}

	if !strings.Contains(output, "static async init(wasmUrl: string)") {
		t.Errorf("expected static init method, got:\n%s", output)
	}
